 ...
 */
public class BusinessInventoryActivityBiz {
    private BusinessInventoryActivity mActivity;
    /**
     * 存放显示在 ListView 中的产品库存数据集合
     */
    private List<BusinessProductStock> mInventoryProductList;
    /**
     * 网络请求支付类型是的标记
     */
    ...
    public BusinessInventoryActivityBiz(BusinessInventoryActivity mActivity) {
       try {
    ...
    private boolean getProductData() {
        try {
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.GetWmsProductZong, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Logger.w(BusinessInventoryActivityBiz.this.getClass() + ".getProductDataSuccess:" + response);
                    getProductDataSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(BusinessInventoryActivityBiz.this.getClass() + ".getProductError:" + error.toString());
                ...
                protected Map<String, String> getParams() throws AuthFailureError {
                  ...
                  //  params.put("strUserId", MyApplication.getInstance().getUser().getIDX());
                    params.put("BusinessCode", MyApplication.getInstance().getBusiness().getBUSINESS_CODE());
                  //  params.put("BusinessCode","ZLG02");
//                  params.put("strPartyidx", mActivity.party_id);
//                  params.put("strOutputIdx", "");
                    params.put("ProductNo", mActivity.productNo);
    ...
    private void getProductDataSuccess(String response) {
        try {
            ...
            if (type == 1) {
                ...
                List<BusinessProductStock> tmpPartyProductStockList = JSON.parseArray(jo.getString("TmsList"), BusinessProductStock.class);
     ...
     * @return 库存产品列表数据
     */
    public List<BusinessProductStock> getInventoryProductList() {
        try {
