 ...
 * 在途订单界面的业务类
 */
public class CheckOrderFragmentIntransOrderPagerBiz {

    private CheckOrderFragmentInTransOrderPage mPager;
    ...
    public CheckOrderFragmentIntransOrderPagerBiz(CheckOrderFragmentInTransOrderPage page) {
        try {
            this.mPager = page;
     ...
     * @return 发送请求是否成功
     */
    public boolean getInTransOrderData() {
        try {
     ...
     * @return 发送网络请求是否成功
     */
    public boolean reFreshIntransOrderData() {
        try {
            mPageIndex = mInitPagerIndex;
            return getOrderData();
        } catch (Exception e) {
     ...
     * @return 发送网络请求是否成功
     */
    public boolean loadMoreIntransOrderData() {
        try {
            return getOrderData();
        } catch (Exception e) {
            ExceptionUtil.handlerException(e);
        ...
        try {
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.GET_ORDER_LIST, new Response.Listener<String>() {
                ...
                public void onResponse(String response) {
                    Logger.w(CheckOrderFragmentIntransOrderPagerBiz.this.getClass() + ".getOrderDataSuccess:" + response);
                    getOrderDataSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(CheckOrderFragmentIntransOrderPagerBiz.this.getClass() + ".getOrderDataError:" + error.toString());
