 ...
 * 已完成订单业务类
 */
public class CheckOrderFragmentCompleteOrderPagerBiz {

    private CheckOrderFragmentCompleteOrderPage mPager;
     ...
     * 网络请求已完成订单数据是的标记
     */
    private final String TAG_GET_COMPLETE_DATA = "TAG_GET_COMPLETE_DATA";

    public CheckOrderFragmentCompleteOrderPagerBiz(CheckOrderFragmentCompleteOrderPage pager) {
        try {
            this.mPager = pager;
    ...
    private boolean getOrderData() {
        ...
        try {
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.GET_ORDER_LIST, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Logger.w(CheckOrderFragmentCompleteOrderPagerBiz.this.getClass() + ".getOrderDataSuccess:" + response);
                    getOrderDataSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(CheckOrderFragmentCompleteOrderPagerBiz.this.getClass() + ".getOrderDataError:" + error.toString());
            ...
            request.setTag(TAG_GET_COMPLETE_DATA);
    ...
    private void getOrderDataSuccess(String response) {
        try {
            ...
            if (type == 1) {
                ...
                List<Order> tmpOrderList = JSON.parseArray(object.getString("result"), Order.class);
    ...
    public void cancelRequest() {
        try {
            HttpUtil.cancelRequest(TAG_GET_COMPLETE_DATA);
        } catch (Exception e) {
            ExceptionUtil.handlerException(e);
        }
    }
