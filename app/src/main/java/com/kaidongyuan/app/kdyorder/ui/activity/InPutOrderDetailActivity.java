...
import com.kaidongyuan.app.kdyorder.adapter.InPutOrderProductListAdapter;
...
import com.kaidongyuan.app.kdyorder.model.InPutOrderDetailActivityBiz;
 ...
 */
public class InPutOrderDetailActivity extends BaseActivity implements View.OnClickListener {
   private String orderIdx;
   private InPutOrderDetailActivityBiz mBiz;
     ...
     * 账单费用条目适配器
     */
    private InPutOrderProductListAdapter mAdapter;
    /**
     * 网络请求是显示的 Dialog
     */
    private MyLoadingDialog mLoadingDialog;
    private InPutOrderInfo minfo;
    private List<InPutOrderProduct> mList;

    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.activity_inputorder_detail);
        initData();
        initView();
        setListener();
        getBusinessFee();
    ...
    private void initData() {
        try {
            if (getIntent().hasExtra(EXTRAConstants.EXTRA_ORDER_IDX)){
                ...
                mBiz = new InPutOrderDetailActivityBiz(this,orderIdx);
            }
    ...
    private void initView() {
        try {
            ...
            mAdapter=new InPutOrderProductListAdapter(null,InPutOrderDetailActivity.this);
    ...
    private void showLoadingDialog() {
        try {
            if (mLoadingDialog == null) {
                mLoadingDialog = new MyLoadingDialog(InPutOrderDetailActivity.this);
            }
    ...
    private void handleRequest() {
        try {
            ...
            tv_OUTPUT_NO.setText(minfo.getINPUT_NO());
            tv_ADD_DATE.setText(minfo.getADD_DATE());
            tv_ADDRESS_NAME.setText(minfo.getSUPPLIER_NAME());
            tv_ADDRESS_INFO.setText(minfo.getSUPPLIER_ADDRESS());
            tv_PARTY_NAME.setText(minfo.getADDRESS_NAME());
            tv_PARTY_INFO.setText(minfo.getADDRESS_INFO());
            mAdapter.setData(mList);
            if (minfo.getINPUT_WORKFLOW()!=null&& minfo.getINPUT_WORKFLOW().equals("新建")
                    && minfo.getINPUT_STATE()!=null&& minfo.getINPUT_STATE().equals("OPEN")){
...
//                }
            ...
            case R.id.bt_confirm:
                try {
                    if (mBiz.confirmOutPutOrder(minfo,mList)) {
                        showLoadingDialog();
                    } else {
                        ToastUtil.showToastBottom(MyApplication.getmRes().getString(R.string.sendrequest_fail), Toast.LENGTH_SHORT);
                    }
    ...
    public void confirmOutPutOrderSuccess() {
        try {
            mLoadingDialog.dismiss();
            ToastUtil.showToastMid("成功确认此入库单，请刷新入库单列表数据！",Toast.LENGTH_LONG);
            this.finish();
        } catch (Exception e) {
            ExceptionUtil.handlerException(e);
        }
