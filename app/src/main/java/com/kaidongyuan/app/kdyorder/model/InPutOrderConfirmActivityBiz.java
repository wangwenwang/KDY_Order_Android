...
import com.kaidongyuan.app.kdyorder.ui.activity.InPutOrderConfirmActivity;
 ...
 * 订单确认界面的业务类
 */
public class InPutOrderConfirmActivityBiz {

    InPutOrderConfirmActivity mActivity;
    /**
     * 获取促销信息是的网络请求标记
     */
    ...
    public InPutOrderConfirmActivityBiz(InPutOrderConfirmActivity activity) {
        ...
        try {
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.SUBMIT_ORDER, new Response.Listener<String>() {
                ...
                public void onResponse(String response) {
                    Logger.w(InPutOrderConfirmActivityBiz.this.getClass() + "getPromotionDataSuccess:" + response);
                    getPromotionDataSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(InPutOrderConfirmActivityBiz.this.getClass() + "getPromotionDataError:" + error.toString());
    ...
    public boolean confirm() {
        try {
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.SaveInput, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Logger.w(InPutOrderConfirmActivityBiz.this.getClass() + "confirmSuccess:" + response);
                    confirmSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(InPutOrderConfirmActivityBiz.this.getClass() + "confirmError:" + error.toString());
                    if (NetworkUtil.isNetworkAvailable()) {
                ...
                protected Map<String, String> getParams() throws AuthFailureError {
                    ...
                    for (int i=0;i<mOrder.OrderDetails.size();i++){
                        ...
                        model.PRICE=mOrder.OrderDetails.get(i).ACT_PRICE;
                        model.INPUT_VOLUME=mOrder.OrderDetails.get(i).PO_VOLUME;
                        model.INPUT_QTY= mOrder.OrderDetails.get(i).PO_QTY;
                        if (mActivity.strInputOrderType!=null&&mActivity.strInputOrderType.equals("input_return")){
                            model.INPUT_QTY=-model.INPUT_QTY;
                        }
                        model.INPUT_UOM=mOrder.OrderDetails.get(i).PO_UOM;
                        ...
                        model.INPUT_WEIGHT= mOrder.OrderDetails.get(i).PO_WEIGHT;
                        ...
                        model.LINE_NO=mOrder.OrderDetails.get(i).LINE_NO;
                        model.PRODUCT_NAME=mOrder.OrderDetails.get(i).PRODUCT_NAME;
                        model.SUM= DoubleArithUtil.mul(model.PRICE,model.INPUT_QTY);
                        inPutOrder.Info.Result.add(model);
                    }
                    inPutOrder.INPUT_QTY= mOrder.TOTAL_QTY;
                    inPutOrder.INPUT_SUM= mOrder.ACT_PRICE;
                    inPutOrder.ADDRESS_CODE=mActivity.strAddressCode;
                    inPutOrder.ADDRESS_INFO=mActivity.strAddressInfo;
                    inPutOrder.ADD_DATE=mOrder.ADD_DATE;
                    inPutOrder.BUSINESS_IDX=mOrder.BUSINESS_IDX;
                    inPutOrder.ADDRESS_NAME=mActivity.strPartyName;
                    inPutOrder.INPUT_WEIGHT= mOrder.TOTAL_WEIGHT;
                    inPutOrder.ADD_USER=MyApplication.getInstance().getUser().getIDX();
                    inPutOrder.ADDRESS_IDX=mActivity.strAddressIDX;
                    inPutOrder.SUPPLIER_CODE="";
                    inPutOrder.SUPPLIER_NAME="";
                    inPutOrder.SUPPLIER_ADDRESS="";
                    if (mActivity.strInputOrderType!=null&&mActivity.strInputOrderType.equals("input_return")){
                        inPutOrder.INPUT_TYPE="采购退库";
                        inPutOrder.INPUT_QTY=-inPutOrder.INPUT_QTY;
                    }else if (mActivity.strInputOrderType!=null&&mActivity.strInputOrderType.equals("input_other")){
                        inPutOrder.INPUT_TYPE="其它入库";
                    }else {
                        inPutOrder.INPUT_TYPE="采购入库";
                    }
                    inPutOrder.OPER_USER=MyApplication.getInstance().getUser().getUSER_NAME();
                    inPutOrder.PRICE= mOrder.ACT_PRICE;
                    inPutOrder.INPUT_VOLUME=mOrder.TOTAL_VOLUME;
