...
import com.kaidongyuan.app.kdyorder.model.MakePreOrderActivityBiz;
 ...
 * 下单界面
 */
public class MakePreOrderActivity extends BaseActivity implements View.OnClickListener, SwipeRefreshLayout.OnRefreshListener, AdapterView.OnItemClickListener {
     ...
     * 对应的业务类
     */
    private MakePreOrderActivityBiz mBiz;
    ...
    private void initData() {
        try {
            ...
            mBiz = new MakePreOrderActivityBiz(this);
    ...
    private void initView() {
        try {
            ...
            this.findViewById(R.id.textview_promotions).setVisibility(View.GONE);
    ...
    private void confirmOrder() {
        try {
            ...
            Intent intent = new Intent(this, PreOrderConfirmActivity.class);
    ...
    public void getProductDataSuccess() {//支付类型，产品品牌类型产品品类，产品数据度获取成功后显示数据
        try {
          ...
          if (mLoadingDialog!=null&&mLoadingDialog.isShowing()){
            ...
            ExceptionUtil.handlerException(e);
            ToastUtil.showToastBottom("展示产品数据失败!", Toast.LENGTH_SHORT);
        }
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        try {
           ...
           if (parentId == mListViewBrands.getId()) {//更换品牌
    ...
    private static class InnerHandler extends Handler {
        private WeakReference<MakePreOrderActivity> makeOrderActivityWeakReference;

        public InnerHandler(MakePreOrderActivity makeOrderActivity) {
        ...
        public void handleMessage(Message msg) {
            try {
                ...
                MakePreOrderActivity activity = makeOrderActivityWeakReference.get();
                if (activity != null) {
                    switch (msg.what) {
         ...
         */
        private void choiceProductDetialIn(Message msg, MakePreOrderActivity activity) {
            try {
                int paddingTopIn = msg.arg1 - MakePreOrderActivity.DETIAL_PICE_PADDING_SIZE_IN;
                activity.mRlChoiceProductdetial.setPadding(0, paddingTopIn, 0, 0);
                if (paddingTopIn > 0) {
                    Message message = activity.mHandler.obtainMessage();
                    message.what = MakePreOrderActivity.WHAT_CHOICE_DETIAL_IN;
                    message.arg1 = paddingTopIn;
                    activity.mHandler.sendMessageDelayed(message, MakePreOrderActivity.DETIAL_IN_PICE_TIME);
                } else {
         ...
         * @param activity MakeOrderActivity
         */
        private void choiceProductDetialOut(Message msg, MakePreOrderActivity activity) {
            try {
                int paddingTopOut = msg.arg1 + MakePreOrderActivity.DETIAL_PICE_PADDING_SIZE_OUT;
                activity.mRlChoiceProductdetial.setPadding(0, paddingTopOut, 0, 0);
                if (paddingTopOut < MakePreOrderActivity.SCREEN_HEIGHT) {
                    Message messageOut = activity.mHandler.obtainMessage();
                    messageOut.what = MakePreOrderActivity.WHAT_CHOICE_DETIAL_OUT;
                    messageOut.arg1 = paddingTopOut;
                    activity.mHandler.sendMessageDelayed(messageOut, MakePreOrderActivity.DETIAL_OUT_PICE_TIME);
                } else {
