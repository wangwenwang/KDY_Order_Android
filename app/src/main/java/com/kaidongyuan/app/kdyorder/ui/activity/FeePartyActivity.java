...
import com.kaidongyuan.app.kdyorder.model.FeePartyActivityBiz;
 ...
 */
public class FeePartyActivity extends BaseActivity implements AdapterView.OnItemClickListener, View.OnClickListener{
    /**
     * 标题栏返回按钮
     */
     ...
     * 下单业务类
     */
    private FeePartyActivityBiz mBiz;
    /**
     * 客户列表 ListView
     */
    ...
    private void initData() {
        try {
            mBiz = new FeePartyActivityBiz(this);
        } catch (Exception e) {
    ...
    private void initView() {
        try {
            ...
            mPartyListAdapter = new PartyListAdapter(FeePartyActivity.this, null);
    ...
    private void showDwonLoadingDialog() {
        if (mLoadingDialog == null) {
            mLoadingDialog = new MyLoadingDialog(FeePartyActivity.this);
        }
    ...
    private void createAddressDialog() {
        try {
            AlertDialog.Builder builder = new AlertDialog.Builder(FeePartyActivity.this);
            ...
            mCustomerAddressAdapter = new AddressListAdapter(FeePartyActivity.this, null);
    ...
    private void jumpToMakeOrderActivity(Address address) {
        try {
            Intent billFeeListIntent = new Intent(FeePartyActivity.this, BillFeeListActivity.class);
            Party party = mBiz.getmSelectedParty();
            billFeeListIntent.putExtra(EXTRAConstants.ORDER_PARTY_ID, party.getIDX());
            billFeeListIntent.putExtra(EXTRAConstants.ORDER_PARTY_NO,party.getPARTY_CODE());
            billFeeListIntent.putExtra(EXTRAConstants.ORDER_PARTY_NAME, party.getPARTY_NAME());
            billFeeListIntent.putExtra(EXTRAConstants.ORDER_ADDRESS_CODE, address.getADDRESS_CODE());
            billFeeListIntent.putExtra(EXTRAConstants.ORDER_PARTY_ADDRESS_IDX, address.getIDX());
            billFeeListIntent.putExtra(EXTRAConstants.ORDER_ADDRESS_INFORMATION, address.getADDRESS_INFO());
            startActivity(billFeeListIntent);
        } catch (Exception e) {
            ExceptionUtil.handlerException(e);
        }
    }
