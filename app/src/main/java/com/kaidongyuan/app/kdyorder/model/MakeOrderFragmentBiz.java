 ...
 * 下单页面的工具类
 */
public class MakeOrderFragmentBiz {

    private MakeOrderFragment mFragment;
    ...
    public MakeOrderFragmentBiz(MakeOrderFragment fragment) {
        try {
        ...
        try {
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.GET_PARTY_LIST, new Response.Listener<String>() {
                ...
                public void onResponse(String response) {
                    Logger.w(MakeOrderFragmentBiz.this.getClass() + "getCustomerData.Success:" + response);
                    getCustomerDataResponseSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(MakeOrderFragmentBiz.this.getClass() + "getCustomerData.VolleyError:" + error.toString());
    ...
    public boolean getPartygetAddressInfo(final int positionInPartysListView) {
        try {
            StringRequest request = new StringRequest(Request.Method.POST, URLCostant.GET_ADDRESS, new Response.Listener<String>() {
                @Override
                public void onResponse(String response) {
                    Logger.w(MakeOrderFragmentBiz.this.getClass() + "getPartygetAddressInfo.Success:" + response);
                    getCustomerAddressResponseSuccess(response);
                }
            }, new Response.ErrorListener() {
                @Override
                public void onErrorResponse(VolleyError error) {
                    Logger.w(MakeOrderFragmentBiz.this.getClass() + "getPartygetAddressInfo.VolleyError:" + error.toString());
                    if (NetworkUtil.isNetworkAvailable()) {
